...
const storageKey = "meta_cnc_hud_state_v2";
const state = { checked: {}, live: { mode: "Réglage", tool: "T08 Ø6", g54: false, feed: 25, api: "SIM", safety: "ATTENTE", toolLength: false, simulation: false, firstPart: false }, sensors: { gps: "GPS: non testé", audio: "Audio: non testé", motion: "Capteurs: simulés PC" } };
...
function onKey(e) {
  ...
  if (e.key === "Escape") { currentStepIndex = 0; selectedCheckIndex = 0; render(); }
...
function getAlert() {
  if (currentStepIndex >= 2 && state.live.tool !== job.expectedToolShort) return ["danger", `⚠ Mauvais outil: ${state.live.tool}. Attendu ${job.expectedToolShort}.`];
  if (currentStepIndex >= 3 && !state.live.toolLength) return ["warning", "⚠ Outil correct mais longueur non mesurée."];
  if (currentStepIndex >= 4 && !state.live.g54) return ["danger", "⚠ Origine G54 non validée."];
  if (currentStepIndex >= 5 && !state.live.simulation) return ["warning", "⚠ Simulation à vide non validée."];
  if (currentStepIndex >= 6 && !state.live.firstPart) return ["warning", "⚠ Première pièce non validée."];
  if (allCriticalOk()) return ["ok", "✅ Feu vert série: conditions critiques validées."];
...
function render() {
  ...
  $("panel-title").textContent = panel === "checklist" ? "Checklist étape" : "Démo lunettes";
  $("panel-mode").textContent = panel === "checklist" ? "M: Checklist" : "M: Démo";
  renderChecklist(); renderDemoPanel(); renderVisual(s.visual);
}
...
function renderDemoPanel() {
  ...
  host.innerHTML = [
    "Gestes clavier: actifs", "Navigation mains libres: active", "Stockage local: actif", `Audio test: ${state.sensors.audio}`,
...
function renderVisual(kind) {
  const map = {
    program:`<svg viewBox='0 0 100 60'><rect x='8' y='6' width='84' height='48' fill='none' stroke='#8fd4ff'/><text x='14' y='22' fill='#d8eeff' font-size='7'>O1205_POCHE_ALU</text><text x='14' y='34' fill='#d8eeff' font-size='7'>N10 G54 G17</text></svg>`,
    vise:`<svg viewBox='0 0 100 60'><rect x='8' y='22' width='84' height='18' fill='none' stroke='#8fd4ff'/><rect x='38' y='14' width='24' height='26' fill='#2c4a66'/></svg>`,
    tool:`<svg viewBox='0 0 100 60'><line x1='50' y1='6' x2='50' y2='44' stroke='#8fd4ff' stroke-width='5'/><polygon points='42,44 58,44 50,55' fill='#8fd4ff'/></svg>`,
    toolLength:`<svg viewBox='0 0 100 60'><line x1='35' y1='8' x2='35' y2='52' stroke='#8fd4ff'/><line x1='55' y1='8' x2='55' y2='52' stroke='#8fd4ff'/><text x='58' y='30' fill='#fff' font-size='7'>L?</text></svg>`,
    g54:`<svg viewBox='0 0 100 60'><line x1='18' y1='46' x2='82' y2='46' stroke='#8fd4ff'/><line x1='18' y1='46' x2='18' y2='10' stroke='#8fd4ff'/><text x='21' y='14' fill='#fff' font-size='7'>G54</text></svg>`,
    path:`<svg viewBox='0 0 100 60'><polyline points='8,46 20,20 35,34 52,16 74,30 92,12' fill='none' stroke='#8fd4ff' stroke-width='2'/></svg>`,
    part:`<svg viewBox='0 0 100 60'><rect x='20' y='14' width='60' height='34' fill='none' stroke='#8fd4ff'/><text x='24' y='32' fill='#fff' font-size='8'>60.04 OK</text></svg>`,
    green:`<svg viewBox='0 0 100 60'><circle cx='50' cy='30' r='18' fill='none' stroke='#40df86' stroke-width='4'/><polyline points='40,30 48,38 62,22' fill='none' stroke='#40df86' stroke-width='4'/></svg>`
  }; $("step-visual").innerHTML = map[kind] || "";
}
function togglePanel() { panel = panel === "checklist" ? "demo" : "checklist"; render(); }
function resetAll() { localStorage.removeItem(storageKey); state.checked = {}; panel = "checklist"; currentStepIndex = 0; selectedCheckIndex = 0; state.live = { mode:"Réglage", tool:"T08 Ø6", g54:false, feed:25, api:"SIM", safety:"ATTENTE", toolLength:false, simulation:false, firstPart:false }; render(); }
function save() { localStorage.setItem(storageKey, JSON.stringify(state)); }
function audioTest() {
  let ok = false;
  try { const ctx = new (window.AudioContext || window.webkitAudioContext)(); const o = ctx.createOscillator(); const g = ctx.createGain(); o.type = "sine"; o.frequency.value = 880; g.gain.value = 0.03; o.connect(g); g.connect(ctx.destination); o.start(); o.stop(ctx.currentTime + 0.14); ok = true; } catch {}
